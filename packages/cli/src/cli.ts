...
async function cmdIndex(): Promise<void> {
  ...
  // Count ATTEMPTS (not map size): a doc whose MDX throws is left out of the map and falls back to
  // plain-markdown html, so map size == ok and the ratio always read N/N — hiding the fallbacks and
  // disagreeing with the "(N docs)" up-to-date line. Report ok/attempts and name the shortfall.
  const total = DOCS.length + variants.length;
  const tag = locales.size ? ` across ${locales.size + 1} locales` : "";
  const fell = total - ok;
  console.log(`kura index: rendered ${ok}/${total} docs via MDX${tag} -> app/_mdx.ts` + (fell > 0 ? ` (${fell} fell back to plain markdown)` : ""));
}
